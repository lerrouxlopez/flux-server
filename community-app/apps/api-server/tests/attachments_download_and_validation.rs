...
async fn attachment_validation_type_denied() {
    ...
    let bad = "data:text/html;base64,PGgxPkhlbGxvPC9oMT4=";
    let req = axum::http::Request::builder()
        .method("POST")
...
#[tokio::test]
async fn attachment_validation_size_denied() {
    if test_database_url().is_none() {
        eprintln!("skipping: set TEST_DATABASE_URL to run integration tests");
        return;
    }

    std::env::set_var("ATTACHMENTS_MAX_BYTES", "4");
    ...
    let data_url = data_url_text_plain("hello"); // 5 bytes
    let req = axum::http::Request::builder()
        .method("POST")
        .uri(format!("/channels/{channel_id}/messages"))
        .header("content-type", "application/json")
...
#[tokio::test]
async fn attachment_download_enforces_tenant_and_membership() {
    if test_database_url().is_none() {
        eprintln!("skipping: set TEST_DATABASE_URL to run integration tests");
        return;
    }

    std::env::set_var("ATTACHMENTS_MAX_BYTES", "1024");
    ...
    let u3 = register(&app, &format!("u3+{}@example.com", Uuid::now_v7())).await;
    let t1 = u1.get("access_token").unwrap().as_str().unwrap();
    let t2 = u2.get("access_token").unwrap().as_str().unwrap();
    let t3 = u3.get("access_token").unwrap().as_str().unwrap();

    // Org1 owned message with attachment.
    let org1 = create_org(&app, t1, &format!("org{}", Uuid::now_v7().simple())).await;
    let ch1 = create_channel(&app, t1, org1.get("id").unwrap().as_str().unwrap()).await;
    let channel1_id = ch1.get("id").unwrap().as_str().unwrap();

    let data_url = data_url_text_plain("hello");
    let req = axum::http::Request::builder()
        .method("POST")
        .uri(format!("/channels/{channel1_id}/messages"))
        .header("content-type", "application/json")
        .header("authorization", format!("Bearer {t1}"))
        .body(Body::from(
            serde_json::json!({
    ...
    // Org2 exists but user2 should not access org1 channel attachment.
    let _org2 = create_org(&app, t2, &format!("org{}", Uuid::now_v7().simple())).await;

    let req = axum::http::Request::builder()
        .method("GET")
        .uri(format!("/attachments/{attachment_id}/download"))
    ...
    // User3 is not a member of org1 at all -> forbidden.
    let req = axum::http::Request::builder()
        .method("GET")
        .uri(format!("/attachments/{attachment_id}/download"))
        .header("authorization", format!("Bearer {t3}"))
        .body(Body::empty())
        .unwrap();
    let res = app.clone().oneshot(req).await.unwrap();
    assert_eq!(res.status(), axum::http::StatusCode::FORBIDDEN);
