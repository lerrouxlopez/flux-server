...
export function WorkShell({ e }: { e: ChannelEngine }) {
  ...
  const showRightSidebar = e.workPane === "pins" || e.workPane === "threads";
    ...
    <div className="grid gap-6 md:grid-cols-[280px_1fr]" data-testid="work-shell">
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-900/30 p-4">
        <div className="flex items-center justify-between">
            ...
            {!e.editingChannelName ? (
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={(ev) => {
                    e.setWorkSearch(ev.target.value);
                    e.setWorkSearchOpen(true);
                    e.setWorkPane("search");
                  }}
                  onKeyDown={(ev) => {
                ...
                {e.workSearch.trim() ? (
                  <button
                    className="rounded-md bg-slate-900 px-2 py-2 text-xs text-slate-200 hover:bg-slate-800"
                    onClick={() => {
                      e.setWorkSearch("");
                      e.setWorkPane(null);
                      e.setWorkSearchOpen(false);
          ...
          <div className="flex items-center gap-2">
            <button
              className={`grid h-8 w-8 place-items-center rounded-md border border-slate-800 px-2 py-1 text-xs ${
        ...
        <div className={showRightSidebar ? "mt-4 grid gap-4 lg:grid-cols-[1fr_380px]" : "mt-4"}>
          <MessageViewport
            e={e}
            density="comfortable"
            panelMode="expanded"
            className="h-[60vh] overflow-auto rounded-lg border border-slate-800 bg-slate-950/30 p-3"
          />

          {showRightSidebar ? (
            <aside className="h-[60vh] overflow-auto rounded-lg border border-slate-800 bg-slate-950/30 px-3 py-2 text-sm text-slate-200">
              <div className="flex items-center justify-between">
                        ...
                        <div className="space-y-3">
                          ...
                          >
                            <TextArea
                              ...
                              onChange={(ev: ChangeEvent<HTMLTextAreaElement>) => e.setThreadDraft(ev.target.value)}
                    ...
                    <>
                      ...
                      >
                        <TextArea
                          ...
                          onChange={(ev: ChangeEvent<HTMLTextAreaElement>) => e.setNewThreadDraft(ev.target.value)}
                          className="flux-input min-h-[60px] flex-1 resize-none rounded-md border px-3 py-2 text-sm outline-none"
                        />
                        <Button
                          className="flux-btn-primary px-3 py-2 text-xs"
        ...
        <Composer e={e} density="comfortable" className="mt-4" />
      </section>
    </div>
  );
}
